...
clz::ClzOpResult clz::OperatorContainer::remove_operator(const QString &signature)
{
...
clz::OperatorBase *clz::OperatorContainer::get_valid_operator(const QString &signature, clz::ClzOpResult &result) const
{
    result = Error;
    if(m_operators.contains(signature)){
...
clz::OperatorBase *clz::OperatorContainer::get_affirm_operator(const QString &signature, clz::ClzOpResult &result) const
{
    result = Error;
    if(m_operators.contains(signature)){
        auto op = m_operators.value(signature);
