...
public:
    ...
    ClzOpResult remove_operator(const QString& signature);
    void remove_operators(const QStringList& signatures);
    void remove_similar_operator(const QString& fragment);

    // 找到一个 "可用" 的对象
    OperatorBase* get_valid_operator(const QString& signature, ClzOpResult& result) const;
    // 找到一个对象
    OperatorBase* get_affirm_operator(const QString& signature, ClzOpResult& result) const;
