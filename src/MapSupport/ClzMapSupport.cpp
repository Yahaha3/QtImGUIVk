...
void clz::ClzMapSupport::clear_layer(const QString &layer_name)
{
    m_map->map_oc()->clear_operators(layer_name);
}
...
void clz::ClzMapSupport::slot_map_support_mark_update(const QJsonObject &jo)
{
    ...
    if(result == ClzSuccess){
        mark->translate_to(GeoPos(lat, lon, alt));
        mark->rotate(yaw);
    }
}
